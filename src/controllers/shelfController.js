...
export const createShelfController = asyncHandler(async (req, res, next) => {
...
export const getShelvesController = asyncHandler(async (req, res, next) => {
  const userId = req.user._id;
...
export const getShelfByIdController = asyncHandler(async (req, res, next) => {
  const userId = req.user._id;
...
export const updateShelfController = asyncHandler(async (req, res, next) => {
  const userId = req.user._id;
...
export const deleteShelfController = asyncHandler(async (req, res, next) => {
  const userId = req.user._id;
  await deleteShelf(userId, req.params.id);
  res.status(200).json(new APIResponse("success", "Shelf deleted successfully"));
});

export const addBookToShelfController = asyncHandler(async (req, res, next) => {
  const userId = req.user._id;
  const { bookId } = req.body;
  const shelf = await addBookToShelf(userId, req.params.id, bookId);
  const response = new APIResponse("success", "Book added to shelf successfully");
  response.addResponseData("shelf", shelf);
  res.status(200).json(response);
});

export const removeBookFromShelfController = asyncHandler(async (req, res, next) => {
  const userId = req.user._id;
  const { bookId } = req.params;
  const shelf = await removeBookFromShelf(userId, req.params.id, bookId);
  const response = new APIResponse("success", "Book removed from shelf successfully");
  response.addResponseData("shelf", shelf);
  res.status(200).json(response);
});
